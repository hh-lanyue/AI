...
from tools import error_tools, global_variable_manger_tool as global_manger
...
def buy_ticket():
    ...
    while not global_manger.get_global_value('is_success') and global_manger.get_global_value('max_buy_times') > 0:
        # 如果没有到车票发售时间则一直循环等待
        while not time_tool.is_begin_buy_ticket(time_mode='local'):
            time_diff, time_wait = time_tool.get_buy_ticket_diff(time_mode='local')
